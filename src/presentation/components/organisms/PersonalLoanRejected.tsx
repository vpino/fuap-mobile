...
export default function PersonalLoanRejected() {
  const {personalLoan} = usePersonalLoanStore();

  const {updateInfoAfterRejected} = usePersonalLoan({
    id: personalLoan.id ?? '',
  });
  ...
  const handleUpdateInfo = async (values: UpdateTermsLoanDto) => {
    setIsLoading(true);
    ...
    <View>
      ...
      <View>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            handleUpdateInfo({amount, duration});
          }}>
