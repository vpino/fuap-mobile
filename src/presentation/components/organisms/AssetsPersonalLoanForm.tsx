...
interface AssetsPersonalLoanFormProps {}

export const AssetsPersonalLoanForm: React.FC<
  AssetsPersonalLoanFormProps
> = () => {
  const {personalLoan} = usePersonalLoanStore();
  const {updateAssets} = usePersonalLoan({id: personalLoan.id ?? ''});
  ...
  const handleAssetsPersonalLoanForm = async (values: UpdateAssetsDto) => {
    setIsLoading(true);
  ...
  return (
    <Formik
      ...
      onSubmit={handleAssetsPersonalLoanForm}>
      {({
        handleSubmit,
        setFieldValue,
        errors,
