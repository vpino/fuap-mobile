...
export default function PersonalLoanApproval() {
  const {individualCustomer} = useOnboardingStore();
  const {personalLoan, resetPersonalLoan} = usePersonalLoanStore();

  const {updateAcceptPersonalLoan} = usePersonalLoan({
    id: personalLoan.id ?? '',
  });
  ...
  const handleAcceptPersonalLoan = async (values: {condition: boolean}) => {
    setIsLoading(true);

    try {
      await updateAcceptPersonalLoan({
    ...
    <View>
      ...
      <View style={styles.container}>
        ...
        <TouchableOpacity
          ...
          onPress={() => {
            handleAcceptPersonalLoan({condition: true});
          }}>
