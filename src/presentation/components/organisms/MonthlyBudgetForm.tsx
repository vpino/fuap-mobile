...
import {LoanDetailsMounthlyDTO} from '../../../infrastructure/dtos/personal-loan/personal-loan.dto';
...
export const MonthlyBudgetForm: React.FC<MonthlyBudgetFormProps> = () => {
  const {id} = useAuthStore();
  const {loadLoanDetailsMounthly} = usePersonalLoan({id: id ?? ''});

  const [snackbarVisible, setSnackbarVisible] = useState(false);
  const [snackbarMessage, setSnackbarMessage] = useState('');
  const [isLoading, setIsLoading] = useState(false);

  const handleMonthlyBudgetForm = async (values: LoanDetailsMounthlyDTO) => {
    setIsLoading(true);
  ...
  const validationSchema = Yup.object().shape({
    ...
    monthlyBills: Yup.string().required(
    ...
    <Formik
      ...
      {({handleSubmit, setFieldValue, touched, errors}) => (
        <View>
          ...
          <SelectList
            setSelected={(value: string) =>
              setFieldValue('monthlyBills', value)
            }
            data={monthlyBillsOptions}
            save="value"
            ...
            notFoundText="No encontrado"
          />
          {touched.monthlyBills && errors.monthlyBills && (
            <Text style={globalStyles.errorText}>{errors.monthlyBills}</Text>
          )}
          ...
          <PrimaryButton
            ...
            disabled={
              !(!errors.monthlyIncome && !errors.monthlyBills) || isLoading
            }
            isLoading={isLoading}
          />
